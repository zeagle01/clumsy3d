...
namespace clumsy
{
	...
	private:
		void compile(const std::unordered_map<uint32_t, std::string>& shader_sources_by_type)
		{
			...
			{
				printf("provide too many shader source %d", int(shader_sources_by_type.size()));
				return;
			}
		...
		static uint32_t get_shader_type_from_string(const std::string& shader_type)
		{
			...
			{
				printf("not support shader type %s\n", shader_type.c_str());
				return 0;
			}
		}
